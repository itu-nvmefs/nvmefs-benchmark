...
for config in "${CONFIGS[@]}"; do
    read -r _ YCSB_SF _ _ _ _ _ <<< "$config"
    ...
    for precond in "${PRECOND_STATES[@]}"; do
        ...
        for ckpt in "${CHECKPOINT_MODES[@]}"; do
            for strategy in "${FDP_STRATEGIES[@]}"; do
                ...
                echo "Running HTAP | TPCH_SF: ${TPCH_SF} | YCSB_SF: ${YCSB_SF} | Mem: ${MEM_LIMIT}MB | NS: ${NS_GB}GB | Temp: ${TEMP_SIZE}GB | Duration: ${DURATION_MIN}min | FDP: ${strategy}"
                TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
                RUN_ID="${BACKEND_LABEL}_${PRECOND_LABEL}_htap_tsf${TPCH_SF}_ysf${YCSB_SF}_mem${MEM_LIMIT}_ns${NS_GB}_ckpt-${ckpt}_fdp-${strategy}_${TIMESTAMP}"
